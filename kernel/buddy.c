...
#define ROUNDUP(n, sz) (((((n)-1) / (sz)) + 1) * (sz)) // Round up to the next multiple of sz 四舍五入到sz的下一个倍数
...
int my_bit_isset(char *array, int index)
{
...
void my_bit_set(char *array, int index)
{
...
bd_malloc(uint64 nbytes)
{
  ...
  my_bit_set(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    my_bit_set(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
void bd_free(void *p)
{
  ...
  for (k = size(p); k < MAXSIZE; k++)
  {
    ...
    //alloc free的时候用的也是set
    my_bit_set(bd_sizes[k].alloc, bi); // free p at size k
    
    if (my_bit_isset(bd_sizes[k].alloc, buddy))
    {        // is buddy allocated?
...
void bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++)
  {
    ...
    for (; bi < bj; bi++)
    {
      ...
      my_bit_set(bd_sizes[k].alloc, bi);
    }
  }
}
